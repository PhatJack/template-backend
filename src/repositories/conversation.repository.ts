...
export async function listByUser(
  ...
  const convs = await ConversationModel.find({ userId })
    .sort({ createdAt: -1 })
...
export async function createConversation(
  ...
  return toConversationResponse(
    created.toObject() as unknown as ConversationRecord,
  );
}
...
export async function deleteConversationCascade(id: string): Promise<void> {
  ...
  try {
    await session.withTransaction(async () => {
      await FileModel.deleteMany({ conversationId: id }).session(session);
      await MessageModel.deleteMany({ conversationId: id }).session(session);
      await ConversationModel.findByIdAndDelete(id).session(session);
    });
  } catch (err) {
    console.error("Error deleting conversation cascade:", err);
  } finally {
    session.endSession();
  }
}
